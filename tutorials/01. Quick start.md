...
```
connection.Instance.please().get({ name: 'myInstance' }).then(function(result, objects) {
  // manipulate the result
});
```

Please note that the callback function can accept an additional parameter (`objects`) that contains an array
